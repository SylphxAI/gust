...
export const parseSize = (size: string | number): number => {
	...
	if (!match?.[1]) return DEFAULT_MAX_SIZE

	const num = parseFloat(match[1])
	const unit = (match[2] || 'b') as 'b' | 'kb' | 'mb' | 'gb'
 ...
 */
export const jsonLimit = (): Wrapper<Context> => bodyLimit({ maxSize: 100 * 1024 })

/**
 * Preset: 10MB limit (for file uploads)
 */
export const uploadLimit = (): Wrapper<Context> => bodyLimit({ maxSize: 10 * 1024 * 1024 })

/**
 * Preset: 50MB limit (for large uploads)
 */
export const largeUploadLimit = (): Wrapper<Context> => bodyLimit({ maxSize: 50 * 1024 * 1024 })
