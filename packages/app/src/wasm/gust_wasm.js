...
export function encode_websocket_close(code, reason) {
	var ptr0 = isLikeNone(reason)
		? 0
		: passStringToWasm0(reason, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc)
...
export function validate_number(value, min, max, is_integer) {
	const ret = wasm.validate_number(
		value,
		!isLikeNone(min),
		isLikeNone(min) ? 0 : min,
		!isLikeNone(max),
		isLikeNone(max) ? 0 : max,
		is_integer
	)
...
export function validate_string(value, min_length, max_length, format) {
	...
	const ret = wasm.validate_string(
		ptr0,
		len0,
		isLikeNone(min_length) ? 0x100000001 : min_length >>> 0,
		isLikeNone(max_length) ? 0x100000001 : max_length >>> 0,
		ptr1,
		len1
	)
	return WasmValidationResult.__wrap(ret)
