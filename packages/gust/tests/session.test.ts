...
describe('Session', () => {
	...
	describe('MemoryStore', () => {
		...
		it('should handle concurrent sessions', async () => {
			...
			const [a, b, c] = await Promise.all([store.get('id-1'), store.get('id-2'), store.get('id-3')])
		...
		it('should handle many concurrent operations', async () => {
			const operations = Array.from({ length: 100 }, (_, i) => store.set(`id-${i}`, { index: i }, 60000))

			await Promise.all(operations)

			const reads = Array.from({ length: 100 }, (_, i) => store.get(`id-${i}`))
