 ...
 */
export const createOtlpExporter = (endpoint: string, headers?: Record<string, string>): SpanExporter => {
  return {
    export: async (spans: Span[]) => {
      ...
      const resourceSpans = [{
        resource: {
          attributes: [
            { key: 'service.name', value: { stringValue: 'serve' } },
          ],
        },
        scopeSpans: [{
          scope: { name: '@aspect/serve' },
    ...
    return async (ctx: Context): Promise<ServerResponse> => {
      ...
      try {
        ...
        span.attributes['http.response_content_length'] = res.body
          ? (typeof res.body === 'string' ? res.body.length : res.body.length)
          : 0
