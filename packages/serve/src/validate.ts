...
import { parseJsonBody, parseFormBody, parseQuery } from './body'
...
export const validateSchema = (
        ...
        if (schema.additionalProperties === false) {
          ...
          for (const key of Object.keys(value as object)) {
            if (!allowedKeys.has(key)) {
              const propPath = path ? `${path}.${key}` : key
              errors.push({ path: propPath, message: 'Additional property not allowed', value: key })
            }
          }
        }
      }
  ...
  if (schema.custom && errors.length === 0) {
    ...
    if (result !== true) {
      errors.push({ path, message: typeof result === 'string' ? result : 'Custom validation failed', value })
    }
  }
 ...
 * URL validator
 */
export const url = (options: Partial<Schema> = {}): Schema =>
  string({ format: 'url', ...options })
...
const validatedDataMap = new WeakMap<Context, { body?: unknown; query?: unknown; params?: unknown }>()
 ...
 */
export const validateBody = (schema: Schema): Wrapper<Context> =>
  validate({ body: schema })

/**
 * Shorthand for query-only validation
 */
export const validateQuery = (schema: Schema): Wrapper<Context> =>
