...
const verifySignature = (data: string, signature: string, secret: string, alg: string): boolean => {
...
export const optionalJwt = (options: Omit<JwtAuthOptions, 'skip'>): Wrapper<Context> => {
	const { secret, algorithms = ['HS256'], issuer, audience, clockTolerance = 0, getToken } = options
