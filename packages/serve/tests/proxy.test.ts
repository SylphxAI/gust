...
import { proxy, getProxyInfo, getClientIp, trustLocalProxy, trustFirstProxy } from '../src/proxy'
...
describe('Proxy Headers', () => {
  ...
  describe('X-Forwarded-For parsing', () => {
    ...
    it('should trim whitespace', () => {
      expect(parseForwardedFor('  192.168.1.1  ,  10.0.0.1  ')).toEqual([
        '192.168.1.1',
        '10.0.0.1',
      ])
    })
  ...
  describe('edge cases', () => {
    ...
    it('should default port based on protocol', async () => {
      ...
      await handlerHttp(createMockContext(
      ...
      await handlerHttps(createMockContext(
    ...
    it('should handle handler throwing error', async () => {
      ...
      await expect(handler(createMockContext(
        { 'x-forwarded-for': '203.0.113.1' },
        '10.0.0.1'
