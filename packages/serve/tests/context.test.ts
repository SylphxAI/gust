...
import { createContext, withParams, parseHeaders } from '../src/context'
...
const createMockParsed = (overrides: Partial<{
  method: number
  path_start: number
  path_end: number
  query_start: number
  ...
  describe('parseHeaders', () => {
    it('should parse headers from buffer', () => {
      ...
      const offsets = new Uint32Array([
        16, 20, 22, 31,  // Host: localhost
        33, 45, 47, 63,  // Content-Type: application/json
      ])
    ...
    it('should lowercase header names', () => {
      ...
      const offsets = new Uint32Array([
        16, 31, 33, 38,  // X-Custom-Header: Value
      ])
    ...
    it('should preserve header value case', () => {
      ...
      const offsets = new Uint32Array([
        16, 29, 31, 44,  // Authorization: Bearer ABC123
      ])
