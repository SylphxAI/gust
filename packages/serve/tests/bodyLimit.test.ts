...
import { parseSize, formatSize, bodyLimit } from '../src/bodyLimit'
...
describe('Body Limit', () => {
  ...
  describe('bodyLimit middleware', () => {
    ...
    it('should use Content-Length header', async () => {
      ...
      const res = await handler(createMockContext('POST', Buffer.from('small'), {
        'content-length': '1000',
      }))
      expect(res.status).toBe(413)
    })
  ...
  describe('edge cases', () => {
    ...
    it('should handle concurrent requests', async () => {
      ...
      const requests = Array.from({ length: 10 }, () =>
        handler(createMockContext('POST', Buffer.from('test')))
      )
