...
// Mock handler
const _mockHandler = async (_ctx: Context) => ({
	status: 200,
...
describe('Cache', () => {
	...
	describe('noCache middleware', () => {
		it('should add no-cache headers', async () => {
			...
			expect(result.headers['cache-control']).toBe('no-store, no-cache, must-revalidate, proxy-revalidate')
	...
	describe('etag middleware', () => {
		...
		it('should generate consistent ETags for same content', async () => {
			const handler1 = async (_ctx: Context) => ({
				status: 200,
				headers: {},
				body: 'Same content',
			})

			const handler2 = async (_ctx: Context) => ({
				status: 200,
				headers: {},
				body: 'Same content',
			})
